...
pub const CipherSuites = struct {
    ...
    pub const ECDHE_RSA_AES256_GCM_SHA384 = struct {
        pub const name = "ECDHE-RSA-AES256-GCM-SHA384";
        pub const tag = @as(u16, 0xC030);
        pub const key_exchange = .ecdhe;
        pub const hash = .sha256;
        pub const prefix_data_length = 8;
        pub const mac_length = 16;
        ...
        pub fn init_state(prefix_data: [8]u8, server_seq: u64, key_data: anytype, header: RecordHeader) State {
            ...
            var iv: [12]u8 = undefined;
            iv[0..12].* = key_data.server_iv(@This()).*;
            iv[4..].* = prefix_data;
        ...
        pub fn raw_write(
            ...
            std.debug.assert(buffer.len <= buffer_size);
            var iv: [12]u8 = undefined;
            iv[0..12].* = key_data.client_iv(@This()).*;
            rand.bytes(iv[4..12]);
    ...
    // all: [3]type = [3]type{ ECDHE_RSA_Chacha20_Poly1305, ECDHE_RSA_AES128_GCM_SHA256, ECDHE_RSA_AES256_GCM_SHA384 },
