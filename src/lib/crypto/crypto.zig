...
pub fn ctr(
    ...
    if (offset != 0) {
        ...
        mem.copyForwards(u8, pad[offset..], src[0..part_len]);
        block_cipher.xor(&pad, &pad, counter);
        mem.copyForwards(u8, dst[0..part_len], pad[offset..][0..part_len]);
        cur_idx += part_len;
        idx.* += part_len;
    ...
    if (cur_idx < remaining) {
        ...
        mem.copyForwards(u8, &pad, src[start_idx..][cur_idx..]);
        block_cipher.xor(&pad, &pad, counter);
        mem.copyForwards(u8, dst[start_idx..][cur_idx..], pad[0 .. remaining - cur_idx]);
